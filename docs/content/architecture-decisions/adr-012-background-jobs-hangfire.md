...
## Status
Accepted. Amended by [ADR 013]({{< relref "adr-013-recurring-job-definitions-and-job-management" >}}): recurring jobs are now `IRecurringJobDefinition`s run through `RecurringJobRunner`, options moved to `JobScheduling:*`, the dashboard is at `/hangfire`, and jobs can be managed through the admin API.
...
## Decision
- **[Hangfire](https://docs.hangfire.io/)** with **Postgres storage** in the application database's own `hangfire` schema (`Hangfire.PostgreSql`). There is no extra infrastructure to run. `JobScheduling:*` options (validated at startup) control whether job scheduling is enabled, the worker count, and whether this process runs a server (see ADR 013).
...
- **Dashboard:** `/hangfire` on the Web API, **Development only** and **local requests only**. It can delete and re-run jobs, so it isn't exposed elsewhere. The Aspire dashboard links to it.
- **Server placement:** the Web API also runs the Hangfire server by default. Set `JobScheduling:RunServer=false` on API instances and run a separate worker host to scale job processing independently.
...
- Hangfire keeps some state (the log provider) in static globals. Functional tests run several hosts at once, so they register a no-op `ILogProvider` (which `AddJobScheduling` honours) and in-memory storage per host, and they run job classes directly instead of starting a server.
